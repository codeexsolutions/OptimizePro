...
public sealed class FabricaDeViewModels(IServiceProvider servicos) : IFabricaDeViewModels
{
    public ViewModelBase Criar(TipoDeTela tipo, object? parametro = null)
    {
        var viewModel = tipo switch
        {
            ...
            TipoDeTela.Impressoras => servicos.GetRequiredService<ImpressorasViewModel>(),
            TipoDeTela.Maquinas => servicos.GetRequiredService<MaquinasViewModel>(),
            TipoDeTela.Historico => servicos.GetRequiredService<HistoricoViewModel>(),
            TipoDeTela.Reposicao => servicos.GetRequiredService<ReposicaoViewModel>(),
            TipoDeTela.Pedidos => servicos.GetRequiredService<PedidosViewModel>(),
            TipoDeTela.OrdensDeServico => servicos.GetRequiredService<OrdensDeServicoViewModel>(),
